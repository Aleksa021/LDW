...
projects its undistorted-pixel lane points into a common bird's-eye view via the
homography H, and decides whether the vehicle is departing its lane.
...
class LDWSystem:
    def __init__(
        ...
        # Vehicle forward axis: image-center column projected to BEV, as a line x=f(y).
        col = np.array([[self.W / 2.0, y] for y in np.linspace(0, self.H_img - 1, 30)],
                       dtype=np.float32).reshape(-1, 1, 2)
        fwd = cv2.perspectiveTransform(col, self.H).reshape(-1, 2)
